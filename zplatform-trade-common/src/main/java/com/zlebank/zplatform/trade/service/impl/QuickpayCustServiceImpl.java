...
public class QuickpayCustServiceImpl extends BaseServiceImpl<QuickpayCustModel, Long> implements IQuickpayCustService{
	...
	public List<QuickpayCustModel> getCardList(String cardNo, String accName,
		 ...
		 List<QuickpayCustModel> cardList = (List<QuickpayCustModel>) super.queryByHQL("from QuickpayCustModel where cardno=? and accname = ? and phone = ? and idnum = ? and relatememberno = ? and devId=?  and  and status = ?", new Object[]{cardNo,accName,phone,cerId,memberId,devId,"00"});
			return cardList;
	}
