...
public class TransferBatchDAOImpl extends HibernateBaseDAOImpl<PojoTranBatch> implements TransferBatchDAO {
	...
	public Map<String, Object> queryTransferBatchByPage(QueryTransferBean queryTransferBean,int page,int pageSize){
		StringBuffer sqlBuffer = new StringBuffer("from PojoTranBatch where 1=1 ");
		StringBuffer sqlCountBuffer = new StringBuffer("select count(1) as total from PojoTranBatch  where 1=1 ");
		...
		resultMap.put("total", queryTransferBatchCount(queryTransferBean, page, pageSize));
		resultMap.put("rows", query.list());
		return resultMap;
	}
	@Transactional(readOnly=true)
	public Long queryTransferBatchCount(QueryTransferBean queryTransferBean,int page,int pageSize){
		StringBuffer sqlCountBuffer = new StringBuffer("select count(*) from PojoTranBatch  where 1=1 ");
		List<String> parameterList = new ArrayList<String>();
		if(queryTransferBean!=null){
			if(StringUtil.isNotEmpty(queryTransferBean.getBatchNo())){
				sqlCountBuffer.append(" and tid = ? ");
