	...
	$.ajax({
		type: "POST",
	 	...
	 	success:function(json){
			...
			if(json=="completed"){
				window.location="<%=basePath %>merch/success.htm?orderNo="+$("#orderNo").val()+"&txnseqno="+$("#txnseqno").val();
			}else if(json=="failed"){
				window.location="<%=basePath %>merch/fail.htm?orderNo="+$("#orderNo").val()+"&txnseqno="+$("#txnseqno").val();
			}else if(json=="wait"){
				window.location="<%=basePath %>merch/wait.htm?reapayOrderNo="+reapayorderno+"&orderNo="+$("#orderNo").val()+"&txnseqno="+$("#txnseqno").val();
			}else if(json=="processing"){
				window.location="<%=basePath %>merch/processing.htm?orderNo="+$("#orderNo").val()+"&txnseqno="+$("#txnseqno").val();
			}else{
				window.location="<%=basePath %>merch/error.htm?orderNo="+$("#orderNo").val()+"&txnseqno="+$("#txnseqno").val();
			}
	 	}
	});
}
