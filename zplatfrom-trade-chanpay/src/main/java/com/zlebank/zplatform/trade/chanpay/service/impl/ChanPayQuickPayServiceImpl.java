...
public class ChanPayQuickPayServiceImpl implements ChanPayQuickPayService{
	...
	public ResultBean protocolSign(TradeBean tradeBean) throws TradeException{
		...
		//记录畅捷实名认证流水
		TxnsWithholdingModel txnsWithholdingModel = new TxnsWithholdingModel(tradeBean.getBankCode(), tradeBean.getBankName(), tradeBean.getCardNo(), tradeBean.getAcctName(), tradeBean.getCardType(), tradeBean.getCertType(), tradeBean.getCertId(), tradeBean.getMobile(), ChannelEnmu.CHANPAYCOLLECTMONEY);
		txnsWithholdingService.saveWithholdingLog(txnsWithholdingModel);
		tradeBean.setPayOrderNo(txnsWithholdingModel.getSerialno());
		//畅捷协议签约
		ResultBean resultBean = chanPayCollectMoneyService.protocolSign(tradeBean);
		if(resultBean.isResultBool()){
	...
	public ResultBean collectMoney(TradeBean tradeBean) throws TradeException{
		...
		//获取渠道对应的绑卡标示
		PojoCardBind cardBind = cardBindService.getCardBind(tradeBean.getCardId(), ChannelEnmu.CHANPAYCOLLECTMONEY.getChnlcode());
		if(cardBind==null){
		...
		TxnsWithholdingModel txnsWithholdingModel = new TxnsWithholdingModel(tradeBean, ChannelEnmu.CHANPAYCOLLECTMONEY);
		txnsWithholdingModel.setSerialno(OrderNumber.getInstance().generateChanPayOrderNo());
		txnsWithholdingService.saveWithholdingLog(txnsWithholdingModel);
		tradeBean.setPayOrderNo(txnsWithholdingModel.getSerialno());
		
		 PayPartyBean payPartyBean = new PayPartyBean(tradeBean.getTxnseqno(),
					"01", tradeBean.getPayOrderNo(), ChannelEnmu.CHANPAYCOLLECTMONEY.getChnlcode(),
		...
		txnsWithholdingModel.setExecmsg(data.getErrMsg());
		txnsWithholdingService.updateRealNameResult(txnsWithholdingModel);
		txnsLogService.updatePayInfo_Fast_result(tradeBean.getTxnseqno(), data.getRetCode(),data.getErrMsg());
		
		if(!"0000".equals(data.getRetCode())){//交易失败
			txnsOrderinfoDAO.updateOrderToFail(tradeBean.getTxnseqno());
			txnsLogService.updateCoreRetResult(tradeBean.getTxnseqno(),"3699", data.getErrMsg());
