 ...
 * @author guojia
 * @version
 * @date 2015年11月17日 上午11:06:43
 * @since 
 */
public class TestQuickTradeThread implements IQuickPayTrade,Runnable{
   ...
   public ResultBean bankSign(TradeBean trade) {
    ...
    public TestQuickTradeThread(){
        ...
        tradeType = TradeTypeEnum.UNKNOW;
        receiveProcessor = (ITradeReceiveProcessor) SpringContext.getContext().getBean("testReceiveProcessor");
        this.tradeBean = null;
    }
    public TestQuickTradeThread(TradeBean tradeBean){
        txnsQuickpayService = (ITxnsQuickpayService) SpringContext.getContext().getBean("txnsQuickpayService");
        testTradeService  = (ITestTradeService) SpringContext.getContext().getBean("testTradeService");
        routeConfigService  = (IRouteConfigService) SpringContext.getContext().getBean("routeConfigService");
        quickpayCustService = (IQuickpayCustService) SpringContext.getContext().getBean("quickpayCustService");
        ...
        tradeBean.setPayinstiId(PAYINSTID);
        receiveProcessor = (ITradeReceiveProcessor) SpringContext.getContext().getBean("testReceiveProcessor");
    }
