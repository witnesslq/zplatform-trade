...
public class ChanPayUtil {
	private static final Log log = LogFactory.getLog(ChanPayUtil.class);
    ...
    public static String generateParamer(Object obj,boolean isIgnoreEmpty ,String[] unParamstring ) throws Exception{
        StringBuffer paramers=new StringBuffer();
        
       Class<?> cls = obj.getClass();
       Class<?>father_class = cls.getSuperclass();
       Field[] father_fields = father_class.getDeclaredFields();
       for (Field field : father_fields) {
           String name = field.getName();
           if("serialVersionUID".equals(name)) continue;
           if(isUnParam(name,unParamstring)){
               continue;
           }
           String strGet = "get" + name.substring(0, 1).toUpperCase() + name.substring(1, name.length());
           Method methodGet = father_class.getDeclaredMethod(strGet);
