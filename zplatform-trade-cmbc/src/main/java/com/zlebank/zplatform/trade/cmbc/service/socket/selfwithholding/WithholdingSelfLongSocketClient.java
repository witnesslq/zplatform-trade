 ...
 * @since
 */
public class WithholdingSelfLongSocketClient extends BaseClient implements Client {

    private static final Log log = LogFactory
            .getLog(WithholdingSelfLongSocketClient.class);
    ...
    private int port;
    private static WithholdingSelfLongSocketClient longClient;

    private WithholdingSelfLongSocketClient(String host, int port, int timeout) {
        this.timeout = timeout;
        this.serverIp = host;
        this.port = port;
    }

    public static synchronized WithholdingSelfLongSocketClient getInstance(String host,int port,int timeout) {
        if (longClient == null) {
            log.info("host:" + host);
            log.info("port:" + port);
            longClient = new WithholdingSelfLongSocketClient(host, port, timeout);
            try {
                longClient.start();
    ...
    class KeepAliveWatchDog implements Runnable {
        public KeepAliveWatchDog(){
            Thread.currentThread().setName("Withholding Self KeepAliveWatchDog Thread");
        }
        long checkDelay = 0;
            ...
            while (running) {
                if (System.currentTimeMillis() - lastSendTime > keepAliveDelay) {
                    try {
                        ...
                        WithholdingSelfLongSocketClient.this.sendMessage("00000000".getBytes(ENCODING));
                    } catch (CMBCTradeException e) {
                        // TODO Auto-generated catch block
                        e.printStackTrace();
                        WithholdingSelfLongSocketClient.this.shutdown();
    ...
    class ReceiveWatchDog implements Runnable {
        public ReceiveWatchDog(){
            Thread.currentThread().setName("Withholding Self ReceiveWatchDog Thread");
        }
        public void run() {
            while (running) {
                try {
